...
# Keyboard KING - Hra na mačkání kláves
...
class App(tkinter.Tk):
    ...
    def vykresli_tecku(self):
        ...
        self.canvas.create_oval(self.pozice_tecky - self.polomer, self.y - self.polomer,
                                self.pozice_tecky + self.polomer, self.y + self.polomer, fill=self.kruh_barva,
                                tag="circle")  # Vykreslení tečky
    ...
    def vykresli_obdelnik(self):
        ...
        for i in range(1, 7):
            # Výpočet x souřadnic pro každý obdélník - cíl space-around layout
            coords = (3 * i - 2) * self.obdelnik_width + self.obdelnik_width
            self.coords_array.append(round(coords + self.obdelnik_width / 2))
            self.canvas.create_rectangle(coords, self.y_obdelniku, coords + self.obdelnik_width,
                                         self.y_obdelniku + self.obdelnik_height, tag=f"{self.klavesy[i - 1]}")

    def show_rules(self):
        """Zobrazení pravidel hry"""
        tkinter.messagebox.showinfo("Pravidla hry", "Pravidla hry:\n\n"
                                                    "1.  hra \"rozsvítí\" v dolní části obrazovky jeden ze šesti "
                                                    "obdélníku, každému obdélníku z leva odpovídají klávesy S D F J K "
                                                    "L.\n\n"
                                                    "2. hra se hraje na přednastavený počet kol (10), přičemž kolo je "
                                                    "vždy naznačeno padajícím barevným kruhem z horní části okna.\n\n"
                                                    "3. jakmile uživatel stiskne po \"rozsvícení\" obdélníku klávesu, "
                                                    "nemůže se již opravit - tento stav je uživateli indikován tak, "
                                                    "že se příslušný obdélník změní na černý.\n\n"
                                                    "4. podaří-li se uživateli stisknout správnou klávesu získá bod, "
                                                    "tento stav je indikován změnou skóre a zároveň barvy padajícího "
                                                    "kruhu na černou).\n\n"
                                                    "5. obdélníky se budou \"přepínat\" v pravidelných intervalech.\n\n"
                                                    "6. Nahoře obrazovky vidíte počet zbývajících kol a dosud "
                                                    "dosažené skóre.")

    def create_labels(self):
        """Vytvoření popisků v horní části okna"""
        # Počet kol
        self.napis = tkinter.Label(self, text=f"Zbývající kola: {self.kola}", font=("Arial", 15, "bold"), fg="white",
                                   bg="blue")
    ...
    def about(self):
        ...
        messagebox.iconbitmap("icon.ico")
        messagebox.geometry("1000x600")

        # Načtení obrázku
        img = PhotoImage(file="authors.png")
        img_label = Label(messagebox, image=img)
        img_label.image = img
        img_label.pack(pady=10)

        # Text s informacemi o nás
        text_label = Label(messagebox, text="↑ O autorech ↑\n\n"
                                            "Hra Keyboard King \n"
                                            "Verze hry: 1.0 \n"
